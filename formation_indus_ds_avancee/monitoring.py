...
def monitor_with_io(
    predictions_folder: str, db_con_str: str, monitoring_table_name: str
) -> None:
    ...
    monitoring_df.to_sql(
        monitoring_table_name, con=db_conn, if_exists="append", index=False
    )
    db_conn.close()
...
def monitor(latest_predictions: pd.DataFrame) -> pd.DataFrame:
    ...
    monitoring_df = pd.DataFrame(
        latest_predictions.groupby("predictions_time").mean(),
        columns=["prediction_time", "mean_prediction"],
    )
